...
?>
    ...
    <main class="content flex-column">
        ...
        if ($resultadoQuery > 0) {
            print "<h3>Último jogo zerado</h3>";

            print "<section style=\"border: 2px solid #282828; padding: 10px; border-radius: 15px;\" class=\"my_game flex-row\">";
            print "<div class=\"img_game\">";
            print "<img src=\"$row->img_url\">";
            print "</div>";
            print "<h1>$row->nome_jogo</h1><h1 style=\"padding:0;\">| $row->horas_jogadas Horas |</h1>";
            print "</section>";
            ...
            print "</div>";
            print "<h1>Nenhum game cadastrado</h1><h1 style=\"padding:0;\">| 000 Horas |</h1>";
            print "</section>";
        }

        print "<h3>Últimos jogos zerados pela comunidade</h3>";

            $mostrarJogos = ("SELECT * FROM jogos ORDER BY id DESC");
            $listarJogos = $conectar->query($mostrarJogos);
            $resultadoJogos = $listarJogos->num_rows;
            $contador = 0;

            if ($resultadoJogos > 0) {
                while (($resLista = $listarJogos->fetch_object()) &&  $contador < 5) {
                    ...
                    print "<h3>Jogador: $resLista->nome_jogador</h3>";
                    print "</div>";
                    print "</section>";
                    $contador++;
                }
